...
def imgToCol(input, height_out, width_out, kernel_size, stride):
...
def colToImg(input, height_pad, width_pad, kernel_size, channel, padding, stride):
...
class ConvolutionalLayer(object):
    ...
    def forward_speedup(self, input):
        ...
        height_out = (height - self.kernel_size) // self.stride + 1
        width_out = (width - self.kernel_size) // self.stride + 1
        self.input_col = imgToCol(self.input_pad, height_out, width_out, self.kernel_size, self.stride)
    ...
    def backward_speedup(self, top_diff):
        ...
        bottom_diff = colToImg(bottom_diff_col, height_pad, width_pad, self.kernel_size, self.channel_in, self.padding, self.stride)
...
class MaxPoolingLayer(object):
    ...
    def forward_speedup(self, input):
        ...
        self.input_col = imgToCol(self.input, self.height_out, self.width_out, self.kernel_size, self.stride).reshape(self.input.shape[0], self.input.shape[1], -1, self.height_out, self.width_out)
    ...
    def backward_speedup(self, top_diff):
        ...
        bottom_diff = colToImg(pool_diff, self.input.shape[2], self.input.shape[3], self.kernel_size, self.input.shape[1], 0, self.stride)
