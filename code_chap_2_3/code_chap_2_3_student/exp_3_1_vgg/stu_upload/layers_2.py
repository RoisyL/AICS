...
class ConvolutionalLayer(object):
    ...
    def forward(self, input):  # 前向传播的计算
       ...
       # 将原始输入数据复制到self.input_pad的中间部分，四周填充零
       ...
       # 初始化一个全零的数组self.output，用于存储卷积层的输出 
       self.output = np.zeros([self.input.shape[0], self.channel_out, height_out, width_out])
       # idxn, idxc, idxh, idxw: 分别代表批次（batch）索引、通道（channel）索引、高度（height）索引和宽度（width）索引
       for idxn in range(self.input.shape[0]):
           for idxc in range(self.channel_out):
               for idxh in range(height_out):
                   for idxw in range(width_out):
                       # TODO: 计算卷积层的前向传播，特征图与卷积核的内积再加偏置
                       # self.input_pad[idxn, :, idxh*self.stride:idxh*self.stride+self.kernel_size, idxw*self.stride:idxw*self.stride+self.kernel_size]: 从输入数据中提取出一个与卷积核大小相同的局部区域，用于与卷积核进行卷积操作
                       # self.weight[:, :, :, idxc]: 选择卷积核中对应于第idxc个输出通道的所有权重
                       # np.sum(self.weight[:, :, :, idxc]*self.input_pad[...]): 这部分计算卷积核与输入数据的局部区域的逐元素乘积的和，即卷积的核心操作
                       # +self.bias[idxc]: 在卷积操作的结果上加上对应于第idxc个输出通道的偏置项。
