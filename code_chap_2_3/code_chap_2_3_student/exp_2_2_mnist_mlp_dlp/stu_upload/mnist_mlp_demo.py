...
class MNIST_MLP(object):
    ...
    def build_model(self, batch_size=10000, input_size=784, 
                    hidden1=512, hidden2=256, out_classes=10, 
        ...
        # BUG：最开始只考虑到了全连接层，没有考虑到其他层，如卷积层，池化层等
        # 在调用接口时一定要认真读参数列表声明和项目原有的修改！不一定和原始列表相同（项目本身可能已经预设了一些参数）
...
def run_mnist():
    ...
    # 注意这里的参数，参数的训练模型中的batch_size（包括其他训练时的数量参数）不需要和此处模型中的batch_size一致，只需要网络结构一致即可
